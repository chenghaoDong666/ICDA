...
def get_image_size(sample: Dict, apply_keys: List[str]) -> List[int]:
    ...
    for image_key in ['image', 'image_ref', 'image_prime', 'image_enhance', 'semantic_enhance']:
...
class ToTensor:
    ...
    def __call__(self, sample):
        ...
        for key in apply_keys:
            val = sample[key]
            if key in ['image', 'image_ref', 'image_prime', 'normalize_mask','image_enhance']:
                sample[key] = torchvision.transforms.functional.pil_to_tensor(
                    val)
            elif key in ['semantic', 'semantic_enhance']:
                sample[key] = torchvision.transforms.functional.pil_to_tensor(
                    val).squeeze(0)
...
class RandomCrop(nn.Module):
    ...
    def forward(self, sample):
        ...
        for key in apply_keys:
            val = sample[key]
            if key in ['image', 'image_ref', 'image_prime', 'semantic', 'normalize_mask', 'image_enhance', 'semantic_enhance']:
...
class ColorJitter(torchvision.transforms.ColorJitter):
    ...
    def forward(self, sample):
        ...
        for key in apply_keys:
            val = sample[key]
            if key in ['image', 'image_ref', 'image_prime','image_enhance']:
                sample[key] = super().forward(val)
            elif key in ['semantic', 'corr_pts', 'corr_pts_ref', 'filename', 'image_prime_idx', 'normalize_mask','semantic_enhance']:
                pass
            else:
                raise ValueError
        return sample
...
class ConvertImageDtype(torchvision.transforms.ConvertImageDtype):
    ...
    def __init__(self, apply_keys='all', scaling=True, **kwargs):
        # torch.float32,因为没有定义,所以就是torch.float
    ...
    def forward(self, sample):
        ...
        for key in apply_keys:
            ...
            if key in ['image', 'image_ref', 'image_prime', 'image_enhance']:
                if random.random() < self.p:
                    sample[key] = super().forward(val)
            elif key in ['semantic', 'corr_pts', 'corr_pts_ref', 'filename', 'image_prime_idx', 'semantic_enhance']:
                pass
            else:
                raise ValueError
        return sample
...
class CompositeFlow(nn.Module):
    ...
    def forward(self, sample):
        ...
        sample['semantic_enhance']=torch.cat([sample['semantic_enhance'].unsqueeze(0)]*3,dim=0)
        flow = trafo(sample)
        if self.add_elastic:
        ...
        for key in apply_keys:
            val = sample[key]
            if key in ['image_prime', 'image_enhance','semantic_enhance']:
                sample[key], sample[key + '_flow'], sample[key +
