...
class Cityscapes(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if isinstance(load_keys, str):
            ...
            self.load_keys = load_keys
        self.enhance_root = self.root.split("/")[:-1]
        self.enhance_root.append("CycleGANCityscapes")
        self.enhance_root = "/".join(self.enhance_root)
        ...
        self.images_enhance_dir = os.path.join(self.enhance_root, 'leftImg8bit', self.split)
        #self.semantic_dir = os.path.join(self.root, 'gtFine', self.split)
        ...
        for city in os.listdir(self.images_dir):
            ...
            img_enhance_dir = os.path.join(self.images_enhance_dir, city)
            for file_name in os.listdir(img_dir):
                for k in self.load_keys:
                    ...
                    elif k == 'image_enhance':
                        file_path = os.path.join(img_enhance_dir, file_name)
                    elif k == 'semantic_enhance':
                        semantic_file_name = file_name.replace(
                            'leftImg8bit.png', 'gtFine_labelTrainIds.png')
                        file_path = os.path.join(
                            semantic_dir, semantic_file_name)
    ...
    def get_rare_class_sample(self):
        assert 'image' in self.load_keys and 'semantic' in self.load_keys and 'image_enhance' in self.load_keys and 'semantic_enhance' in self.load_keys
