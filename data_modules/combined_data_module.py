...
class CombinedDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        # print(self.batch_size) batch_size为4,分到每一个就是2
    ...
    def setup(self, stage: Optional[str] = None):
        if stage in (None, "fit"):
            ...
            for ds, cfg in zip(self.train_on, self.train_config):
                # 调用此处的全局变量
                # 最终调用的是类,如:<class 'data_modules.datasets.cityscapes.Cityscapes'>
    ...
    def on_before_batch_transfer(self, batch, dataloader_idx):
        if self.trainer.training:
            ...
            for sub_batch in batch:
                # cityscapes: 'filename','image','semantic','image_enhance','semantic_enhance'
                # darkzuric: 'filename','image','imageref'
                if 'semantic' in sub_batch.keys():  # supervised semantic batch
                    src_inp.append(sub_batch['image'])
                    src_y.append(sub_batch['semantic'])
                    img_enhance.append(sub_batch['image_enhance'])
                    semantic_enhance.append(sub_batch['semantic_enhance'])
            ...
            if len(src_inp) > 0:
                ...
                new_batch['image_enhance'] = torch.cat(img_enhance, dim=0)
                new_batch['semantic_enhance'] = torch.cat(semantic_enhance, dim=0)
            if len(trg_inp) > 0:
